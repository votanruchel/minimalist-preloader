        ...
        <form method="post" action="options.php">
            ...
            <table class="form-table">
                <tr valign="top">
                    ...
                    <td><input type="color" name="minimalist_loader_spinner_color" value="<?php echo esc_attr( get_option('minimalist_loader_spinner_color','#000000') ); ?>"></td>
                </tr>
                <tr valign="top">
                    <th scope="row">Cor do Fundo do Spinner</th>
                    <td><input type="color" name="minimalist_loader_spinner_bg_color" value="<?php echo esc_attr( get_option('minimalist_loader_spinner_bg_color','#ffffff') ); ?>"></td>
                </tr>
                <tr valign="top">
                    <th scope="row">Cor do Fundo da Tela</th>
                    <td><input type="text" name="minimalist_loader_background_color" value="<?php echo esc_attr( get_option('minimalist_loader_background_color','rgba(255,255,255,0.8)') ); ?>" placeholder="rgba(255,255,255,0.8)"></td>
                </tr>
                <tr valign="top">
                    <th scope="row">Usar blur no fundo?</th>
                    <td>
                        <select name="minimalist_loader_use_blur">
                            <option value="1" <?php selected(get_option('minimalist_loader_use_blur','1'),'1'); ?>>Sim</option>
                            <option value="0" <?php selected(get_option('minimalist_loader_use_blur','1'),'0'); ?>>Não</option>
                        </select>
                    </td>
                </tr>
                <tr valign="top">
                    <th scope="row">Tempo mínimo de exibição (ms)</th>
                    <td><input type="number" min="0" name="minimalist_loader_min_time" value="<?php echo esc_attr( get_option('minimalist_loader_min_time','1000') ); ?>"></td>
                </tr>
                <tr valign="top">
                    <th scope="row">Tempo máximo de exibição (ms)</th>
                    <td><input type="number" min="0" name="minimalist_loader_max_time" value="<?php echo esc_attr( get_option('minimalist_loader_max_time','5000') ); ?>"></td>
                </tr>
                <tr valign="top">
                    <th scope="row">Evento do Google Ad Manager para aguardar</th>
                    <td>
                        <input type="text" name="minimalist_loader_gpt_event" value="<?php echo esc_attr(get_option('minimalist_loader_gpt_event','')); ?>" placeholder="Ex: slotRenderEnded">
                        <p class="description">
                            Caso queira aguardar um evento GPT específico (ex: <code>slotRenderEnded</code>), insira aqui.  
                            Consulte a <a href="https://developers.google.com/publisher-tag/reference?hl=pt-br#googletag.events.SlotRenderEndedEvent" target="_blank">documentação do Google Publisher Tag</a> para ver outros eventos disponíveis.
                            Deixe vazio para usar apenas o carregamento inicial (<code>googletag.apiReady</code>) e o tempo máximo.
                        </p>
                    </td>
                </tr>
            </table>
...
function minimalist_loader_show_preloader() {
    ...
    (function(){
        ...
        var minTime = <?php echo $min_time; ?>; 
        var maxTime = <?php echo $max_time; ?>;
        var startTime = Date.now();
        var gptEvent = "<?php echo esc_js($gpt_event); ?>";
        ...
        function conditionsMet() {
            ...
            var adManagerReady = (typeof googletag !== 'undefined' && googletag.apiReady === true);
        ...
        if (gptEvent) {
            var interval = setInterval(function(){
                if (typeof googletag !== 'undefined' && googletag.apiReady === true && googletag.pubads) {
                    ...
                    // Registrar o listener do evento
